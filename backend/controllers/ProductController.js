...
const getAnProductById = (req, res) => {
  ...
  const query = `SELECT title,rating,counter,products.id,description,firstName,user_id, users.users_image,products.image,price,category FROM users INNER JOIN products ON users.id=products.user_id where products.id = ? and products.is_deleted =0 and products.is_deleted =0 and users.is_deleted = 0`;
